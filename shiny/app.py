...
app_ui = ui.page_fluid(
    ui.navset_tab(
        ...
        ui.nav_panel("Model",
            ...
            ui.p("A pitcher can control or influence features of the pitch such as initial speed, spin, position, velocity, and acceleration, but has less ability to influence the play after that. This model uses these features at the time of the pitch to predict the position of the ball when it reaches the front of the plate. Based on those position coordinates, the model then aims to predict the tendencies of the outcomes of the pitch (ball, hit, or strike), thereby creating a link between the part of the play that it entirely within the pitcher's control and the overall outcome."),
            ui.p("First, the position of the ball over the front of the plate was determined. However, not every pitch reaches the front of the plate if, for example, the ball makes contact with the bat ahead of the plate. Additionally, coordinate data is not necessarily recorded at the exact moment that the y-coordinate was 0. Therefore, the x- and z-coordinates at the time point when the y-coordinate was zero were extraploted. A randomly selected handful of pitches' recorded positions and the extrapolated coordinate when the y-coordinate is zero are shown below."),
            ui.output_image("y0_extrap"),
            ui.p("While the extrapolated coordinates are not technically data, based on the trajectories of the balls, it is reasonable to assume that this is where the ball would have ended up if not acted upon by the bat. These coordinates can effectively be predicted using the initial features of the pitch. Over 20 trials with 75% of the data used as training data and 25% of the data used as test data, the average R-squared value for the predicted x-coordinate as compared to the true x-coordinate in the test set was 0.984 with a standard deviation of 0.0016. The average R-squared value for the predicted z-coordinate as compared to the true z-coordinate in the test set was 0.983 with a standard deviation of 0.0119."),
            ui.p("The position over the plate was then used to predict the outcome of the pitch. These variables are less cleanly related, as there is more unpredictability that is out of the pitcher's control. Shown below is the outcomes of pitches in the data set based on the extrapolated x- and z-coordinates when the y-coordinate was zero."),
            ui.output_image("color_results"),
            ui.p("Three methods were tested in their efficacy of predicting the pitch result based on the coordinates: Decision Tree, Gradient Boosting Machine, and K Nearest Neighbors. Over 5 trials using 75% of the data as training data and 25% of the data as test data, K Nearest Neighbors tended to perform the best."),
            ui.output_image("model2_compare"),
            ui.p("After determining that the K Nearest Neighbors Model should be used, the number of neighbors to use was further optimized. To this end, 50 trials were run using a Grid Search Cross Validation. While there was a lot of variation, the mean best number of neighbors was 34.96, so we will use 35 neighbors to fit the model."),
            ui.output_image("num_neighbors"),
            ui.p("Over 20 trails in which 75% of the data was training data and 25% of the data was test data, the mean R squared value was 0.683 with a standard deviation of 0.0268. The final model was trained on the entire data set using 35 neighbors for K Nearest Neighbors model. The user can input any combination of features to yield a prediction of the outcome of the play."),
        )
    )
)
